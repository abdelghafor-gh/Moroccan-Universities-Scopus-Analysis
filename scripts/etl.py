...
def create_directories():
    ...
    Path(project_root / "data/transformed").mkdir(parents=True, exist_ok=True)
...
def extract_affiliation_name(affiliation_full_name, city, affiliations_by_city, universities_by_city):
    ...
    if city in affiliations_by_city:
        for key, value in affiliations_by_city[city].items():
            if key in affiliation:
                return key, value

    if city in universities_by_city:
        for key, value in universities_by_city[city].items():
            if key in affiliation:
                return key, value
